...
```{r}
model <- lm(alertness ~ treatment_group + focus_difficulty + physical_tiredness + hours_since_wake, data = df_compliant)
summary(model)

```
The linear regression shows that participants who delayed their coffee had **0.32** higher alertness scores on average compared to the control group, although this effect was only **marginally significant** (p = 0.079). The **standard error was 0.17**, which means there’s **moderate uncertainty** around this estimate.
